...
class DeltaParserGroup {
	constructor(options) {
		...
		this.filters = [
			this._actionChatFixError,
			this._actionConnectedFixError,
		].map(fn => fn.bind(this));
	}
	...
	_parseSymbol(message, key, data) {
		...
		if ( tbData ) {
			if ( key !== "*") {ret.push([`${message.table}:${key}`, tbData, key]);}
			ret.push([`${message.table}:*`, tbData, key]);
			if ( this.noSymbol ) {ret.push([`${message.table}`, tbData, key]);}
		}
		
		return ret;
	}
	
	_actionChatFixError(message, key, data, deltaParser) {
	...
	_actionConnectedFixError(message, key, data, deltaParser) {
		if ( message.table === "connected" && message.action === "update" && !deltaParser.getData() ) {
			deltaParser.action({
				table: message.table,
				action: "partial", 
				keys: [],
